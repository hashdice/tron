...
contract HashDice_HDT {
  HashDiceTRC20I private _trc20;
  //--------------------------------------------------------------------------------------------------
  // constants.
  uint constant JACKPOT_MODULO = 1000;

  uint constant MAX_AMOUNT = 100000000 * (10 ** 9);  //100,000,000 hdt;
  ...
  uint public maxProfit = 50000 * (10 ** 9);  //50,000 hdt
  uint public minBet = 20 * (10 ** 9);        //20 hdt
  uint public houseEdge = 15;                 //1.5%
  uint public minHouseEdge = 1 * (10 ** 9);   //1 hdt

  uint public minJackpotBet = 500 * (10 ** 9);//500 hdt
  uint public jackpotFee = 5 * (10 ** 9);     //5 hdt

  struct Bet {
    // 投注额(以hdt decimal为单位, 1 hdt = 10 ** 9 hdt decimal).
    uint amount;
    // 游戏类别(取模值).
    uint8 modulo;
    // 赔率 (* modulo/rollUnder),
  ...
  constructor () public{
    ...
    _trc20 = HashDiceTRC20I(address(0x0));                                                                       
  }
